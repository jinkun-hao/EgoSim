...
from egosim_state.streams.frame_dir_stream import FrameDirStream
from egosim_state.utils.logging import configure_logging
...
@click.option(
    ...
    default=Path.cwd() / "egosim_state_results",
)
...
def infer(video: Path, image_dir: Path, output: Path, pipeline: str, visualize: bool):
    ...
    egosim_state_pipeline = make_pipeline(args.pipeline)
    ...
    egosim_state_pipeline.run(video_stream)
    logger.info("Finished")


@click.command()
@click.argument("data_path", type=click.Path(exists=True, path_type=Path), default=Path.cwd() / "egosim_state_results")
...
def main():
    """NVIDIA Video Pose Engine (EgoSim State) CLI"""
    pass
