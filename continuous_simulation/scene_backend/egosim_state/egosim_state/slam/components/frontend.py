...
class SLAMFrontend:
    ...
    def __update(self):
        ...
        # Never remove the last video frame keyframe - ensure it stays in dense_disp_frame_inds
        ix_to_remove = self.t1 - 2
        total_n_frames = self.args.get("total_n_frames", -1)
        is_last_frame_kf = (
            total_n_frames > 0
            and ix_to_remove < self.video.n_frames
            and int(self.video.tstamp[ix_to_remove].item()) == total_n_frames - 1
        )
        ...
        if d.max().item() < self.keyframe_thresh and not is_last_frame_kf:
            self.graph.rm_second_newest_keyframe(ix_to_remove)
            self.t1 -= 1
        else:
