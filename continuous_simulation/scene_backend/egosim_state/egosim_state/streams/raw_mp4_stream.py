...
from egosim_state.streams.base import ProcessedVideoStream, StreamList, VideoFrame, VideoStream
...
class RawMp4Stream(VideoStream):
    ...
    def __init__(self, path: Path, seek_range: range | None = None, name: str | None = None) -> None:
        ...
        # Fix: If start is beyond video length (e.g., from filename like "0_121_241.mp4" 
        # where video only has 121 frames), reset to 0 to read the actual content
        if self.start >= _n_frames:
            self.start = 0
            self.end = _n_frames
