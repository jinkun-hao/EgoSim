...
def convert_egosim_state_to_colmap(artifact: ArtifactPath, output_path: Path, depth_step: int, use_slam_map: bool):
    """Convert EgoSim State reconstruction results to COLMAP format."""

    logger.info(
        f"Converting EgoSim State results from {artifact.base_path} ({artifact.artifact_name}) to COLMAP format at {output_path}"
    )
...
def main():
    """Main function for EgoSim State to COLMAP conversion script."""
    parser = argparse.ArgumentParser(
        description="Convert EgoSim State reconstruction results to COLMAP format",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("egosim_state_path", type=Path, help="Path to EgoSim State results directory")
    parser.add_argument(
        "--sequence",
        "-s",
        type=str,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory for COLMAP format (default: <egosim_state_path>_colmap)",
    )

    args = parser.parse_args()

    if not args.egosim_state_path.exists():
        print(f"Error: EgoSim State path '{args.egosim_state_path}' does not exist.")
        return 1

    # Find artifacts
    artifacts = list(ArtifactPath.glob_artifacts(args.egosim_state_path, use_video=True))
    if args.sequence is not None:
        artifacts = [artifact for artifact in artifacts if artifact.artifact_name == args.sequence]

    # Set default output path
    if args.output is None:
        args.output = args.egosim_state_path.parent / f"{args.egosim_state_path.name}_colmap"

    for artifact in artifacts:
        convert_egosim_state_to_colmap(artifact, args.output / artifact.artifact_name, args.depth_step, args.use_slam_map)
    return 0
