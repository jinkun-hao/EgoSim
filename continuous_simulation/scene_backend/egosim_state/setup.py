...
# Prefer the conda-provided nvcc only when the full toolkit is present.
if "CONDA_PREFIX" in os.environ:
    conda_prefix = os.environ["CONDA_PREFIX"]
    conda_nvcc_path = os.path.join(conda_prefix, "bin", "nvcc")
    conda_cicc_path = os.path.join(
        conda_prefix, "targets", "x86_64-linux", "nvvm", "bin", "cicc"
    )
    if os.path.exists(conda_nvcc_path) and os.path.exists(conda_cicc_path):
